<div align="center">

![Header](https://capsule-render.vercel.app/api?type=waving&height=245&color=0:111827,45:0f766e,100:22c55e&text=HUNG%20LE&fontColor=ffffff&fontSize=54&fontAlignY=40&animation=fadeIn)

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=23&duration=2500&pause=800&color=22C55E&center=true&vCenter=true&width=900&lines=Backend-minded+Developer;Java+%2B+JavaScript+%2B+Python;Desktop+Apps+and+Web+Systems;Applied+AI+Experiments" alt="Typing SVG" />

### Portfolio profile updated on July 5, 2026

[![GitHub](https://img.shields.io/badge/GITHUB-Hungle2910-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Hungle2910)
[![Gmail](https://img.shields.io/badge/GMAIL-lhung29100%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:lhung29100@gmail.com)
[![Profile Views](https://komarev.com/ghpvc/?username=Hungle2910&style=for-the-badge&color=22c55e)](https://github.com/Hungle2910)

</div>

---

## About Me

<table>
<tr>
<td width="68%">

```yaml
Name: Hung Le
GitHub: Hungle2910
Location: Vietnam

Focus:
  - Backend and database-backed systems
  - Java Servlet/JSP MVC applications
  - Desktop productivity apps
  - Applied AI and classification experiments

Working With:
  - Java, JavaScript, Python
  - SQL Server
  - Git and GitHub
  - Clean README and portfolio documentation

Current Projects:
  - Hung's To-Do List Desktop App
  - Motorcycle Sales Appointment System
  - Quantum-Inspired NFR Classification
```

</td>
<td width="32%" align="center">

<img src="https://media.giphy.com/media/SWoSkN6DxTszqIKEqv/giphy.gif" width="260" alt="Coding animation" />

</td>
</tr>
</table>

---

## Tech Stack

<div align="center">

![Java](https://skillicons.dev/icons?i=java)
![JavaScript](https://skillicons.dev/icons?i=js)
![Python](https://skillicons.dev/icons?i=py)
![Electron](https://skillicons.dev/icons?i=electron)
![HTML](https://skillicons.dev/icons?i=html)
![CSS](https://skillicons.dev/icons?i=css)
![Git](https://skillicons.dev/icons?i=git)
![GitHub](https://skillicons.dev/icons?i=github)
![VS Code](https://skillicons.dev/icons?i=vscode)

<br />

![SQL Server](https://img.shields.io/badge/SQL%20Server-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white)
![Servlet JSP](https://img.shields.io/badge/Servlet%20%2F%20JSP-007396?style=for-the-badge&logo=openjdk&logoColor=white)
![MVC](https://img.shields.io/badge/MVC-0F766E?style=for-the-badge&logo=codeigniter&logoColor=white)

</div>

---

## GitHub Dashboard

<div align="center">

<img height="165" src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=Hungle2910&theme=github_dark" alt="GitHub stats" />
<img height="165" src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=Hungle2910&theme=github_dark" alt="Repositories per language" />

</div>

---

## Contribution Graph

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Hungle2910&theme=github-dark&hide_border=true&area=true&custom_title=Hung%20Le%27s%20Contribution%20Graph" alt="Contribution graph" />

</div>

---

## Contribution Snake

<div align="center">

![Contribution snake](https://raw.githubusercontent.com/Hungle2910/Hungle2910/output/github-contribution-grid-snake-dark.svg)

</div>

> The snake is generated from real GitHub contribution data by the workflow in this profile repository.

---

## Featured Projects

### Hung's To-Do List Desktop App

```text
JavaScript | Desktop Application | Productivity
```

A desktop app project focused on simple personal task management and practical JavaScript app structure.

- Builds a small productivity workflow around daily tasks.
- Good place to show UI polish, packaging, and app-state improvements.
- Current public project to keep improving with screenshots, releases, and setup notes.

[![Repository](https://img.shields.io/badge/Repository-To--Do%20Desktop%20App-181717?style=for-the-badge&logo=github)](https://github.com/Hungle2910/hungs-to-do-list-desktop-app)

---

### Motorcycle Sales Appointment System

```text
Java | Servlet/JSP | MVC | SQL Server
```

A web system for motorcycle sales management and showroom appointment scheduling.

- Organizes sales and appointment workflows in an MVC-style Java web app.
- Uses SQL Server for persistent business data.
- Shows backend flow design, database-backed screens, and practical domain modeling.

[![Repository](https://img.shields.io/badge/Repository-Motorcycle%20Sales%20System-181717?style=for-the-badge&logo=github)](https://github.com/Hungle2910/motorcycle-sales-appointment-system)

---

### Quantum-Inspired NFR Classification

```text
Python | Classification | Applied AI | Requirements Engineering
```

An applied AI experiment exploring non-functional requirement classification with quantum-inspired ideas.

- Focuses on classification for software requirement analysis.
- Connects machine learning experimentation with software engineering research.
- Strong candidate for clearer experiment notes, metrics, and reproducible setup.

[![Repository](https://img.shields.io/badge/Repository-Quantum%20NFR%20Classification-181717?style=for-the-badge&logo=github)](https://github.com/Hungle2910/Quantum-Inspired-NFR-Classification)

---

## Developer Skills

<div align="center">

| Backend | Desktop / UI | Database | AI / Data | Tools |
|---|---|---|---|---|
| Java | JavaScript | SQL Server | Python | Git |
| Servlet / JSP | Electron-style apps | Relational design | Classification | GitHub |
| MVC structure | HTML / CSS | CRUD workflows | Experiment notes | VS Code |
| Business workflows | Usable interfaces | Data modeling | Evaluation habits | Documentation |

</div>

---

## 2026 Goals

- Improve Java backend structure and MVC project quality.
- Add screenshots, setup guides, and demo notes to portfolio repositories.
- Polish the to-do desktop app into a stronger public project.
- Strengthen SQL Server database design and workflow modeling.
- Make applied AI experiments easier to reproduce and explain.
- Build a consistent, real contribution history through useful project updates.

---

## Connect With Me

<div align="center">

[![GitHub](https://img.shields.io/badge/GITHUB-Hungle2910-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Hungle2910)
[![Gmail](https://img.shields.io/badge/GMAIL-lhung29100%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:lhung29100@gmail.com)

</div>

---

<div align="center">

Thanks for visiting my profile.

</div>
